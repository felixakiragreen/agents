...
defer, charge doc); the amendment law from the amender's side (D57 — round 4
proved the field refuses bare messages); the standing side-quest grant at
judgment altitude (F16); the usage-gauge clause in escalation triggers (F16's
routing); the flow world (D73 — tender naming, the dead Dispatcher gone);
D64/D74 baton shapes. What left: the epigraph (the door owns identity); the
Dispatcher-tended default (D71); dead vocabulary; the duplicated tier-guard/
precedence sections (one summons paragraph, shared verbatim with the Digger).*
...
2. **Verify landings:** each LANDED charge's contract — findings filed
   where its doc says, status current, commits present, holds typed (D74).
...
5. **Rule or escalate:** settle what the evidence settles; escalate what
   it doesn't. Sweep the inbox.
