# The Digger mantle — redraft v4 (the tightening pass)

*⬡'s ask: one more redundancy look. Cuts, each with its reason: "a documented
kill is a win" + "a claim without evidence is a draft" — the door owns both
pinned formulas, the charter keeps only its deltas; the fork-stop restatement
trimmed (door's first contract, cited not re-said); bulletin read/write duties
merged into one bullet, protocol left to doctrine §9 and the coda; "scope
pressure is a side-quest" bullet cut (duplicated the side-quests section);
forbidden list 7 → 6 (two bullets overlapped); "the workhorse of dispatched
batches" cut (Architect-facing); D64/D74 cites folded into the §11 pointer.
Body ~30% shorter than v3.*
