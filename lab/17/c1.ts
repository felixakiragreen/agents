// lab/17 — C1: Felix's hand. Replays three real landed changes (plus the countersign
...
//   R3  GA-11 ledger append   — one session entry                (commit daa0ca3)
//
...
// geometry, no history surgery. The S arm edits lab/17/twin/ only and restores via git.
...
function sArm(before: (() => void) | null, edit: () => void, reverse: boolean): Metrics {
	if (before) { before(); execSync('git add lab/17/twin', { cwd: REPO }); }
	edit();
	const m = measure(`git diff ${reverse ? '-R ' : ''}-U0 -- lab/17/twin`,
		`git diff ${reverse ? '-R ' : ''}-U0 --word-diff=porcelain -- lab/17/twin`);
	execSync('git checkout HEAD -- lab/17/twin', { cwd: REPO });
	return m;
}
...
replays.push(['R2b D63 countersign', 'amendment mark trued', mArm('f3cd47c', 'DECISIONS.md'), sArm(null, () => {
	...
	e.body = e.body.replace('· ✓ Felix same day: where a', '— proposed, pending Felix countersign: where a');
	save('decisions.json', d);
}, true)]);

replays.push(['R3 GA-11 ledger append', '1 session entry', mArm('daa0ca3', 'LEDGER.md'), sArm(null, () => {
	const l = twin('ledger.json');
	if (l.at(-1).date !== '2026-08-28') throw new Error('tail is not the GA-11 entry');
