...
#   render.zsh <sandbox> <columns> <mantle-key> <model> <effort> <account-key> <keys> [yank] [bump]
# The name-stamp's lineage ordinal comes from the sandbox's own log, counted once here as the
# panel counts it at open; $PWD is the theater, so callers cd to the theater they mean.
set -u
source $1/summon.zsh 2>/dev/null
_summon_load || { print -u2 "render: $_summon_error"; exit 1 }
# the v1.1 baseline this harness also renders predates the counter, hence the guard
