...
| 18 | [v3 · the great re-cut](plans/18-great-recut.md) — full-corpus migration via `doctrine migrate`, history included | 16 | Dispatcher · sonnet-medium (tends an 8-row wave of scoped Architects) | OPEN — **cut 2026-08-26 (GA-10)**; supersedes D32's history scope for form (D63); DoD: `doctrine lint ~/code` → 0 failures across the register (a rule, not a list — 22 buildings today); re-scoped by 17 only if 17 wins |
