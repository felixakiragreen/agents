...
outside its venue's project root; if `auto` prompts on out-of-tree writes, the step
pauses at its 60-minute timeout, visibly in the Works — that pause is flow-1
evidence (P5's next cell), and the fallback is one rig summons. Trust cells measured
