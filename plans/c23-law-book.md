...
The canon speaks the standard. The blessed vocabulary (D71 ⬡✓) becomes the law book's
own tongue: DOCTRINE, the global file's waggle line (a live wire ×3), the templates,
the composition law, the city glossary — and the standard itself is promoted into
canon. Nothing changes meaning; every word changes owner. Form molts freely under
D63's molt clause; meaning is stone.
...
- `canon/CLAUDE.md` — **live ×3: editing it IS deploying it.** D71 is the signature;
  signed canon may touch the wire.
- [canon/mantles/README.md](../canon/mantles/README.md) · [docs/the-city.md](../docs/the-city.md)
...
2. **DOCTRINE.md — the full respell.** The graveyard is the mapping; quotes stay
   ...
   when narrating history (period terms survive only inside quotation marks).
   - §13 → a short pointer: the working vocabulary is the Guild's Standard
     (`canon/work/STANDARD.md`, D71); the glossary that lived here is superseded.
   - Wherever a pinned formula's principle appears, the text carries the pinned string
     exactly (§1's "Append, fold, strike" → "Append, distill, strike." — all 24 are in
     the standard §8).
   - §4: row→charge; Felix-gate→⬡-gate in both column grammars; PARKED→DEFERRED;
     `unstaffed` dies — **charges are always staffed**, and Staffing `—` is legal only
     on a DEFERRED charge (standard §6); C‹n› ids named with a pointer at the standard's
     namespace law; "the glass renders his card" → Belvedere/the deck; the parked
     list → the deferred list.
   ...
   - §11: the baton keeps D64's grammar; the shapes speak the standard (§3: a baton
     hands an **action**; shapes **single / batch / fork**).
   - §12: keel language → cornerstone (lay the cornerstone).
3. **canon/CLAUDE.md — the live wire.** The waggle line remade per the standard §7:
   *"Waggle me X" means: the shortest possible plain-speech explanation — one sentence
   per thing.* The four-slot anatomy and the §3 contract pointer die — the line now
   carries the whole contract. **⬡-fork, in-session:** add one pointer line for the
   standard (bare sessions have no other wire to it — "The Guild speaks a standard:
...
4. **canon/mantles/README.md** — grammar and law, not a charter; the charters are
   ...
   - Summons grammar verbs respelled ("execute the charge doc | run the board | review
     the batch").
...
6. **canon/work/templates/**: `brief.md` + `order.md` → **`charge.md`** — one skeleton
   (Mission/Questions · Inputs · the genre core by mantle: digging = questions + kill
   criteria, building = blessed spec + `Done when:` · Out of scope · Findings · kickoff
   fenced). The other templates (map, ledger, decisions, issues, claude-md) respell
   their tokens (⬡-gate, DEFERRED, charge ids, `Done when:`).
7. **docs/the-city.md**: §3's waggle remade (the one-sentence contract; lineage kept,
   trimmed); §1's table respelled (rows→charges, job ticket→the charge doc, the foreman
