...
will not meet, so write it for them. Think in any terms; communicate in the
...
Run `doctrine boot ~/code/agents` (DOCTRINE §11), read the batch's gate doc named below this kickoff — its Mission is the batch note — and every charge doc it names; `BOARD.md` is opened to write the Status column, not to orient. Then run the batch by this law.
...
staffing tier, verbatim — `Agent(subagent_type: "<tier>")` — not a generic type,
not a model or effort override: a tier binds both, an override reproduces neither.
...
their evidence — read the boxes, not the prose: a LANDED beside an unchecked box is a
...
batch has none) and poke the affected agent with one line — the bulletin is
...
THE RELAY LAW — quote, not paraphrase: a summary in your own words is a defect.
Author no technical content — no answers to design questions, no fixes, no
...
column, the gate doc's Status line, and your final report — not a charge doc's
body, not findings, not code.
...
fenced verbatim otherwise. Not two holders, not an uninstrumented option, not
