...
**Status:** LANDED 2026-08-31 — REJECTED (E1 ruled by Felix the same day: close 029,
ignite 037 — the salvage landed at `6b795b5`, 215 green; the E1 block below stands as
the record; header reconciled to the row at the grand-architect-19 sweep) · **Depends on:** — ·
**Staffing:** Architect · opus-high ·
**Parallel-safe with:** 036
...
> many words — because master repaired itself after 025. The branch is 1-red (F2) and
> superseded in every conflicting hunk (F3), so D48 forbids the merge and the rejection
> is ruled here (F5). What waits on him is one word: the 029 status ruling and whether
> the one salvageable piece — the removal arm, laid as [037](037-removal-arm.md) and
> already proven 215 green — is worth landing. `bv/029-summon-harness` @ `f160ec1` is
> kept, not deleted: the reject stays reversible.

## Mission

029's landing comes home. The summon-harness repair is complete on
`bv/029-summon-harness` @ `f160ec1` and was stranded when agents-flow-1 was
abandoned (belvedere D22) — G1, the gate that would have merged it, is KILLED.
Master's `lab/008` harness has stood 15-red since 025's preset retirement. Verify,
merge, reconcile.
...
1. **Control first:** run `./lab/008/run` on master — expect the 15-red (proves the
   harness can fail; a green control here is itself an escalation).
2. **Verify on the branch:** run `./lab/008/run` from a checkout of
   `bv/029-summon-harness` — the run has FINISHED green before any merge.
3. **Merge:** `git merge --no-ff bv/029-summon-harness`. The known collision is
   `LEDGER.md` — the branch carries 029's own entry. Resolve by keeping master's
   tail intact and appending the branch's 029 entry at the tail verbatim, with a
   dated note that the merge landed 2026-08-31. Append-only law: no history
   rewrite, no force-push, ever.
4. **Post-merge:** `./lab/008/run` on master — green, pasted. Verify 029's charge
   doc arrived reading LANDED; reconcile the BOARD.md rows — 029 →
...
- [ ] BOARD.md rows reconciled — evidence and recommendation on both rows; the status
      ruling rides E1. `bv/029-summon-harness` deliberately **kept** (F5).
- [x] Ledger appended (this gate's entry).

## Out of scope

- `presets.tsv` changes and new harness arms — 029's F4 (the typed-literals arm)
  stays a finding for the next charge that opens `lab/008`. Creep is a bug.
...
itself, by the three summon commits that followed 025's preset retirement:
...
`67d73be`: 211 PASS · 0 failure(s).**  *(Corrected 2026-08-31 at 037's landing: this
...
was tuned to reach it. 037-F2.)* 029's mission — "`lab/008/run` goes green again on
...
Precedent for a charge's bar cleared by later hands on master: 031's row (its one unmet
...
029's `Done when:` accepted this as "the charge's pre-existing environmental failure"
...
029 spec item 4 (013-F1's guard gains its removal arm — "the arm that would have caught
...
Master 210 + 5. The salvage is proven, not predicted — laid as [037](037-removal-arm.md)
...
- **`bv/029-summon-harness` @ `f160ec1` is NOT deleted.** The spec authorised deletion
  only after a merge; there was none. It stands until 037 lands, and the reject stays
  reversible.
- **Escalated (E1)** — the charge doc's own instruction: "a green control here is itself
  an escalation." What is Felix's is the status ruling on 029, recorded below as a
  recommendation, not taken.

**Recommendation (one word closes it):** 029 → `LANDED — the mission met on master by
later hands (7973440 · 08a97d0 · 6724213); branch REJECTED, superseded; the removal arm
→ 037`. G2 → `LANDED — REJECTED f160ec1`. The alternative, if he would rather the arm
die with the branch: kill 037 and delete the branch — the harness stands 210 green
