...
- **The suite and the lint, at HEAD:** `bun test` from `doctrine/` green; `doctrine lint
  ~/code/agents` names only belvedere's nineteen `board.cell-cap` (a retired building's
  books, D84 — that red stands until the belvedere purge on the deferred list); any new
  failure 046's arms raise on a live building is named on this row and stays red.
- **Each landing against its own bar** — the pasted evidence re-run, not re-read: the packs
  and their sizes (044), the census and the typed tail (045), the fixtures and their
  controls (046). A bar met by weakening a check is a rejection.
- **Three rulings, the Architect's — reversible and cited, no word of his owed:**
  - **(i) The pack's size.** If `doctrine boot` at a building exceeds 8 KB because its tail
    entry is long (`ledger.entry-cap` warns at 150 words; stigmergon's entries run past
    400), the defect is the entry's, not the pack's: 044 LANDS with the finding, and the
    pack is never trimmed — the verbatim law (formula 9) outranks the size bar.
  - **(ii) A live board reddened by `board.gate-kickoff`.** The rows are named in the
    findings and stay red; the arm is never weakened and no building's doc is edited from
    this gate — its Architect owns the fix (a canon parser reads; it does not patch).
