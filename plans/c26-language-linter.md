...
**Status:** LANDED 2026-08-29 · **Depends on:** C24 · **Staffing:** Builder · opus-high
...
which C25 measured and laid as C30. Default-on would have buried five real form failures
...
**The surfaces it reads (spec item 5), against C25's live list:** a building's boards (master
docs included), its own `CLAUDE.md`, and its OPEN / IN FLIGHT / BLOCKED charge docs. Two
deliberate narrowings from C25's list, both stated rather than assumed: **`README.md` only at
a building's own anchor** (a nested package's README is not a law surface, and the city has
hundreds), and **the inbox is not read at all** — C25 swept the ISSUES *header*, but an entry
...
C25 typed are legal. No new class. The five that stand are whiteboardy's board-truth defect,
still that building's Architect's at its gate 26, exactly as C25 left them.
...
**4. The city run, post-C25** — `doctrine lint --vocab ~/code`:
...
**6. `doctrine lint ~/code/agents` — 0, exactly as C24 left it** (form arms, default). Under
...
- **A LANDED or KILLED board row is history whole.** Not just its Status annotation (C25's
  ...
  class in this repo's own MAP (charges 0, 05, 18, 21, C23, C25).
- **`canon/` is fenced.** §9 IS a table of dead words; a law book that may not name the dead
  cannot bury them. C23 respelled it under Felix's sign-off and C25 fenced it for the same
  reason.

**And the ruling that makes the arm livable: a mention is spelled in ticks or quotes.** The
arm cannot tell use from mention — C23-F3 said so and C25 laid C30 on exactly that
...
Not fixed here: it is C24's parser, the widening changes `decisions`, an entity total the
...
`lab/21/ortho.ts`'s `ISE_OK` omits `improvise`, `advertise` and `supervise` — all three sit in
...
- **13 in MAP.md** — 4 `Dispatcher` tombstone mentions · `the keel sitting` and `the glass` in
  two OPEN charges' own titles (20 and C27) · `the glass` again in a batch note · a bare `the
  register` · a `PARKED` mention · a `fire` · `colour` ×3.
- **31 in OPEN charge docs** — 18's eleven (`Row 16`, `cut`, `DoD`, `the register`) · 20's five
  `keel`s and its `Dispatcher` · 22's five · 11's three `fire`s · C27's two `the glass` ·
  C28's and C30's `Dispatcher`s.
...
register` 2 · `cut` 2 · `row 16`/`Row 16`/`Row 17`/`row-16` 4 · `PARKED`, `DoD`, `behaviour`,
`synchronise` 1 each.

C25's claim was **grep-clean on the surfaces it finished**, and where the tool can check that
...
named here rather than counted as a pass.) Every one of the 44 is on a surface C25's own
...
not name (11, 18, 20, 22, C27, C28, C30 — it named 11, 20 and 22 for the *machine* tokens
only), and **9 spelling hits, which no charge has ever swept**: D71 §8's spelling ruling
landed with the standard and C25 was the graveyard's sweep, not the lexicon's.

So: the form run is 0, the tool agrees with C25 where C25 spoke, and "already respelled" was
true of less than it sounded.

**Ruled, and this is the escalation:** the bar met here is the form run at 0. The vocabulary
backlog for this repo is 44 hits needing use-vs-mention adjudication — the same method C30
owns for the outer city — and it is **filed as a ticket, not swept**, because sweeping it is
editing building text this charge is forbidden to touch. Felix's call whether it rides C30,
becomes C31, or waits.

### F4 — charge 18's own doc still reads OPEN, three days after the board landed it

`plans/18-great-recut.md`'s `**Status:**` line says `OPEN`, so `isLiveWorkDoc` reads it as a
live surface and the arm lints its history (12 hits). The MAP says **LANDED** (C25 reconciled
it). One of the two is lying, and by C25's own findings it is the doc. A one-line repair, in
...
after this charge — both reds are C25's `d dispatcher` preset retirement (`colourOf(rig,
...
preset, which is C27's business.
...
anchor only, because C25's live list holds surfaces that staff nobody. No entity total moved:
