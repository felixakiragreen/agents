...
- Belvedere's board shows the molt rows LANDED, their tests green per their own
  agreements.
- The deck renders the standard's tokens against a live corpus (this repo's board is
  one — it carries ⬡-gate, C‹n›, and DEFERRED by then).
- The visual pass is his — recorded as the annotation.

## Out of scope

Canon files. Other buildings. New deck features — this is a vocabulary molt only;
creep is a bug.

## Findings

**LANDED 2026-08-29 — code, renders, and tests whole; the ⬡ visual pass is the one
open end and rides this Status as the annotation when given (spec §5).** The molt ran
at home as **Belvedere batch 7** (their board §6, their agreements): local charges
**C1** (the fence repoint — their own inbox ruling, riding along) · **C2** (the
vocabulary molt — this commission's content) · **C3** (a flake their C2 found, killed
as the batch's amendment). Belvedere declared the C‹n› namespace at this batch
(standard §2/§7); P/B/G ids grandfathered (D18).

### Done when — measured

1. **Belvedere's board shows the molt LANDED, tests green per their agreements:**
   three landings, zero kills; `belvedere/glass` **669 tests green in one process**
   (651 → 669), offline type gate exit 0, `doctrine` 71/0 — every gate re-run at this
   Architect session's own hand, not taken from the landing reports.
2. **The deck renders the standard's tokens against a live corpus:** the render proof
   is their C2 F3's fixture PAIR — the same four documents in the graveyard's grammar
   and the standard's, one substitution apart by construction, asserting
   **byte-identical rendered HTML** through the same functions the server serves
...
- **Render vocabulary (spec 2):** the needs-you queue is **the ⬡-queue** by name
  (pane, buttons, tooltips); copy speaks ignite · charge · batch · DEFERRED ·
  `D71: single · batch · fork`; countersign copy → **bless/blessing** (the gesture's
  appended line molts to `bless D‹n›: ✓` while `recordedIn` reads both heads
  forever); the self-name is **the deck**. Their F1 ruling, worth canonizing some
...
1. **The type gate was RED at HEAD since canon C26, and nothing watched.** C26
   ...
   (`9d0ee05`). The general fact: `doctrine/` has exactly one consumer building, the
   deck's offline `bunx tsc --noEmit` covers both (B8), and **no doctrine charge runs
   it** — a shape change lands in one building while its breakage sits in another.
   Ask: doctrine-touching charges add the deck's type gate to their bar, or the
...
`fa48e83`→`46f67dd` C2 (eight) · `7ab4f8a`+`9e8c5e9`+`15d372c` C3 · board
reconciliations and close. Findings: `belvedere/plans/c1-fence-repoint.md` ·
`c2-vocabulary-molt.md` · `c3-grep-clock.md`; relays in `belvedere/plans/BULLETIN.md`.

**⬡ visual pass:** *pending — the annotation lands on the Status line above at his
word; start with a deck relaunch (the running process predates the molt).*
