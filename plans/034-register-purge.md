...
The constitution consolidated at ~D76 — Felix's call, moved up from the
D100 deferral (F26, reconfirmed at F32: "SO much cruft"). Three arms, one
sitting: **the cut** (the D52-lane leakage retired into amendments of
ancestors — the census is on file, ~7–10 entries, F31), **the form
migration** (the historical `✓ Felix` marks and pre-D63 grammars re-emitted
in the current grammar, meaning byte-preserved — the molt clause's biggest
exercise yet), and **the taste recalibration** (every surviving entry
re-read against current-Felix — the calibration sweep run over the whole
register at once; strain lands as amendment candidates for his batch
blessing).
...
Sweep oldest → newest; classify each entry per the three branches; draft
the consolidated register in a working file before touching the live one;
present the whole migration as one blessing with the kill list, the
amendment list, and the untouched list separated. Prior numbering stands —
ids are addresses forever (D18); a retired entry keeps its number and gains
its tombstone line.
...
The city's other registers (each building's own Architect). The standard
(D71 stands). Any new law — this sitting only consolidates.

## Findings

- **F1 — the census verdict (2026-08-29).** All 76 entries read whole; every law's
  claimed home verified by reading the home, never the entry's "Edits:" line. ~97%
  of the register was already distilled — DOCTRINE, STANDARD, the charters, the
  mantles README, MAP, the global file carry it all. Four homeless clauses found:
  the tier maxim (never escalate tier to compensate for incomplete orders — was
  D29's), the serialization law (was D65's), the ancestry test for every building's
  register (was D52's), the Directives governance line (was D66's). One pending
  entry: D72. Everything else was legislative history of law living elsewhere.
- **F2 — the method amended at the desk (⬡, this sitting).** Tombstone lines
  killed: a killed entry DELETES — a gap in the numbering is a killed entry, git
  holds every byte; amendments distill into the entry text, no scar owed.
  Supersedes this doc's method note ("a retired entry keeps its number and gains
  its tombstone line"); ids still never reused, nothing renumbers. The three arms
  collapsed into one rewrite: the form-migration arm dissolved (nothing historical
  survived to migrate), the recalibration arm ran as the walk itself. The
  going-forward law is DOCTRINE §8's re-cut: the ancestry test, meaning changes
  rewrite the entry at his blessing (ledger names it, git holds the old), the
  purge clause.
...
- **F4 — dangling references ruled (⬡ asked).** History stands untouched — closed
  ...
  and the molt clause's meaning-stone forbids rewriting them anyway. The
