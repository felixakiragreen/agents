...
hits). D63's amendment was countersigned *after* row 016 built, so every typed absence
...
which is the diff row 016's findings §DoD-4 publish as proven form-only. 18b already
...
**030's re-measure of this repo's live vocabulary set is taken against a moving board.**
030's fence forbids it from editing any charge doc that is IN FLIGHT or in agents-flow-1's
own batch (029 · 030 · 031 · 032 · G1) — their hits are filed to `~/code/agents/ISSUES.md`
...
**031 landed a NEW lint arm mid-flow, so the city's failure count moves for reasons no
other lane caused.** `10f4012 doctrine: the kickoff arm — a live charge's fence must open
by the door (031 item 4)`. 030's baseline at ignition was `5 failure(s) in 1 class(es)`
...
`kickoff.door`, all in `plans/*.md` files 030 never opened. **G1: do not read a risen
count as a 030 or 029 regression.** The honest comparison for any prose lane is
per-building, per-class, against a baseline taken with the SAME `doctrine/` revision.
Evidence: `git -C ~/code/agents log --oneline` (10f4012); 030's own two landings moved
zero form-arm failures (whiteboardy 5 → 5, snappy's README 0 new).

### 2026-08-29 · 031 → 032 (the next lane on this checkout) and G1
**Four things 031 changed under `doctrine/` that 032 builds on top of, and one number G1
must not read as a regression.**

1. **A decision id is now spelled ONCE** — `DECISION_ID` in `src/grammar.ts`, read by the
   candidate test, the head match and both migrate decision rules. 032 mints grammar
   tokens: add them there, not at the use site, or 026-F1's drift returns.
...
4. **`doctrine lint ~/code` went 6 → 26 failures and NONE of the 20 new ones is a doc
   regression** — they are `kickoff.door`, 031's new arm reading live charge docs' fences
   for the first time (snappy 9 · simmy 4 · manny worktree 3 · cap-mega docs/units 2 ·
   belvedere 1 ×2 copies), filed at `~/code/agents/ISSUES.md`. Same finding as 030's entry
   above, from the tool's side. **G1: `doctrine lint ~/code/agents` reads 3, not the
   charge's 0** — 029's live `ledger.baton` (present under the pre-031 tool too) and
   belvedere C1's pre-door fence counted twice. Both named in 031's F5/DoD 2; neither is
   inside 031's fence.

**And a live one for whoever next runs `doctrine migrate` anywhere:** the false
`unrecorded` clause fills are dead (whiteboardy's pre-025 ledger: 66 fills → 4, all four
verified honest), but `round-trip ok` still means only "declared fields", exactly as 025-F1
