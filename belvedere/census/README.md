...
Ten Claude Code hook events → one JSONL line each, appended to `$CENSUS_DIR/census.jsonl`
...
worktree: `bun belvedere/census/deploy.ts --check` is the read-only drift alarm ×3;
