...
| B1 | [Census deploy](plans/b1-census-deploy.md) — the liveness sensor live ×3 accounts | — | Builder · opus-high | **LANDED** 2026-08-26, branch `bv/b1-census` — four files, no escalation. A live hooked session emits **14 F6-shaped records**, key list byte-identical to F6 and in F6 order, subagent calls separated by `aid`; the control venue stayed silent (file never created). Bench **p95 6.46 ms** on the fattest of P1's 87 captures (2158 B) — 1.5× under the bar; hardening costs +0.54 ms, the price of dropping `exec`. Every failure mode (no jq, unwritable dir, dir deleted mid-session, malformed payload) exits 0 with **zero bytes on both streams** — and the same breakage REFUSES `deploy.ts`, which probes the hook live on every run and asserts the record against F6. Fixture proofs green incl. both refusal paths; **plan-then-apply** means one bad account writes nothing anywhere. `bg` capped at 16 (P1 F6's named guard). **Live ×3 deploy PENDING Felix at G1** — read-only `--check` says all three accounts are hook-free, so the merge is purely additive. Ritual must run from the merged checkout, never a worktree |
| B2 | [Glass spine](plans/b2-glass-spine.md) — bun server: City View + building pages over `doctrine/` + census | — | Builder · opus-high | IN FLIGHT — dispatched 2026-08-26, worktree `bv/b2-glass` |
