...
- **Two-lane commit rule (B15 F7 + P6's confirmation, folded at G2 2026-08-28):**
  in a multi-lane batch on one shared checkout, a row commits by **explicit
...
| B23 | [The repaint law](plans/b23-repaint-law.md) — the tenant-listener leak (candidate 6; the verdict-card parity face), the Chat's scroll as repaint-preserved state, the Act-pane stall (candidate 8, hypothesis — reproduce first) | — | Builder · opus-high | OPEN |
...
| B27 | [QoL sweep + close flow](plans/b27-qol-close.md) — the Works' Chat hotswap wired, the honest-disabled law city-wide, the pass gesture findable at flow level, the account-at-arm label, the tweak list absorbed (STOP-clause bounded), `flow-close-6` left unarmed | B24 (last — sweeps all batch-6 surfaces) | Builder · opus-high | OPEN |
| G3 | Rework gate — verify batch 6's landings, fold, sweep ISSUES, cut next; fires by the armed close flow | B27; Felix-gate: the PermissionRequest ritual + the trust-entry drain + the rework visual pass | Architect · fable-high | OPEN |
...
sitting's own hand: **651 tests green in one process, offline type gate exit 0**;
...
2026-08-28 in the G2 sitting**, formalized at `flow-close`'s card 16:39:47Z.
**Batch 5 CLOSED 2026-08-28** — this is the final line, written by the recording
sitting the pass fired: his arm in the Works (one click, fifteen `armed` lines
...
sweep — `attemptFire`'s post-create addressing moves to uuids (P6 F2 ratified,
...
2026-08-28, ruled at the recording sitting):** repo-vs-campaign differentiation
...
**Batch 6 (the deck rework, cut 2026-08-28 at the batch-6 cut sitting — his
direction: the field reports ARE the chapter; ⬡ stays parked, Ava stays v0.5) —
strictly serial on master, Dispatcher-tended (D61), announce duty (D67), rider
applies:** **B23 → B22 → B26 → B25 → B24 → B27 → G3**, one row in flight,
...
entries land in ISSUES / the desk until B27 fires, and the Dispatcher hands
...
asks ride findings and the batch report. **Blessing (Felix, pending):** three
pre-chewed rulings are his to strike — arrangement persistence as a desk
file (B24 §2), building-homed placement with mint-on-ambiguity (B25 §1), the
account-at-arm label-not-knob (B27 §4). **G3 fires by the armed close flow**
...
countersigns · the D64 baton-grammar asks are batched into canon row 20 — B26
