...
| C21 | [The gut: the cmux nag](plans/c21-gut-v1.md) — the `nagging` idle-prompt class dies whole with its stale labels; presence SURVIVES on the census's own sensors (his correction, verbatim in the doc) | — | Builder · opus-high | **LANDED** 2026-08-31 — all four bars, budget 0: the class is gone from type, rank, badge, dot, note, legend, CSS and test (four grep hits left, every one a comment recording the removal), the 21-hour NAGGING row now reads **IDLE** and the ⬡-queue went 5 → 4 while presence stayed byte-identical (5 live, WORKING and BLOCKED unmoved). The fixture city gained the state it lacked so the gut has a standing subject, and `nag-honesty.probe.ts` joins the `--probes` family, seen to fail. Findings F1–F4 in [C21](plans/c21-gut-v1.md); F1 is a live warning for G6 — **the PermissionRequest ritual re-opens the blindness this closed** unless `waitingOf` learns the event |
