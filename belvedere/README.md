...
> **RETIRED 2026-08-31 (⬡ — his ruling: "the attempt lacked the right
> foundation").** The campaign is KILLED with its books closed — a documented
> kill is a win. The successor is **stigmergon** (founding opened 2026-08-31):
> the same dream, built ground-up this time — the post-mortem lives in the
> successor's cornerstone non-goals, where failure lessons are load-bearing.
> **The salvage:** the v3 engine · console · fake claude · barrage
> ([v3/](v3/README.md)), the camera + gates + fixture city, the census, the
> distilled laws of §5, and the desk (`~/code/agents/desk/` outlived this
> renderer — the files-carry-truth law proven on Belvedere's own grave).
> **Known gaps riding the salvage:** baton renderers must handle
> `holder ∈ {dispatch, none}` (C36 F4, swept from the inbox at the close);
> census `waitingOf` reads only `Notification` and misses `PermissionRequest`
> (C21 F1) — the waiting blindness re-opens on headless steps until fixed.
> Nothing below this notice is current; it stands as the record.
...
- **A desk-writing surface's bar runs twice** (B24's seam, ruled at its
  review): pointer gestures prove against the camera's twin, the file
  against a real Belvedere — the camera never grows a real-desk mode
  (C17's disarm law stands; a probe clicking a live deck ignites real
  sessions).
- **A probe that drives a write asserts the receipt** (B24 F3/F4): the
  client's optimistic repaint makes refused writes invisible — a page whose
  every write 400s looks exactly like one whose writes landed; read the
  file or the wire receipt, never the paint.
...
| B26 | [Batons on Belvedere](plans/b26-baton-attention.md) | — | Builder · opus-high | **LANDED** 2026-08-31 — the baton is the fifth attention class in the one computation (badges are the queue, bucketed; `baton` interleaves with `waiting` under one law): `glass/baton.ts` split out of the rail, per-option compose/copy with zero ignite wiring, the Works' landed terminal node hands its baton; glass 609 → 621, the fixture-sid fix (item 0) landed first; findings F1–F9 in [B26](plans/b26-baton-attention.md). Reviewed at the tender's hand: the filings ruled — F6 (the scrim stacking bug) → B24's first act, F9 (ComposeDraft has no tier) → B27 §7, F2 (a grammar word for "nothing is owed", fourth filing) + the Next-splitter mis-split → the canon inbox; the proving run re-run at the review (evidence below the batch note) |
...
| B24 | [His arrangement](plans/b24-arrangement.md) — the City fully his: reorder, relabel, recolor, nest; persists as `desk/city-arrangement.json`; truth underneath (census decides existence, attention outranks recency inside); candidate 7's verdict recorded either way; the layer is **spaces** (his model, ruled at the blessing — doc header carries the sketch) | — | Builder · opus-high | **LANDED** 2026-08-31 — the City is his: one recursive `Space` (reorder/relabel/recolor/nest by pointer drag), persisted at `desk/city-arrangement.json`, truth underneath; candidate 7 REPRODUCED on the live register (7 sections for 5 labels) and fixed through his arrangement's own renderer; the scrim (item 0) closed first with its standing probe; glass 621 → 657; findings F1–F11 in [B24](plans/b24-arrangement.md). Reviewed at the tender's hand: the two filings distilled into §5 (the two-run desk bar; a driving probe asserts the receipt), inbox swept; the builder's one ruling — attention outranks his order at every level — ACCEPTED on §3's own law (strike-able at the G6 visual pass, one comparator line, the file keeps his order either way); his live :4477 runs pre-B24 code until his next restart |
| B27 | [The QoL sweep](plans/b27-qol-close.md) — hotswap wired, honest-disabled city-wide, locked-tooltip dismissal, the decoder's scope walk + planned-mention resolve, the fake/real filter (C15 F5), the account knob with three labeled windows (his ruling 2026-08-28), surface titles, the tweak list (STOP-bounded) | B22; B23; B24; B26; C21 | Builder · opus-high | **KILLED** 2026-08-31 — the building retired at stigmergon's founding (⬡); the tweak list dies with the surfaces it swept; the un-ruled residue (the locked-tooltip dismissal, the decoder's scope walk, C15 F5's fake/real filter, C21 F1's PermissionRequest blindness + F4's legend token) reads from its doc for any salvage need |
...
| G6 | The rework close — verify the batch's landings, distill, sweep the inbox, lay next | B27; ⬡-gate: the rework visual pass + the PermissionRequest ritual (B22's runbook, Felix-run) | Architect · fable-high | **KILLED** 2026-08-31 — superseded by the retirement (⬡); the close-out session flipped the books in its stead (this board's ledger tail is the record) |
