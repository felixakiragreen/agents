...
| B24 | [His arrangement](plans/b24-arrangement.md) — the City fully his: reorder, relabel, recolor, nest; persists as `desk/city-arrangement.json`; truth underneath (census decides existence, attention outranks recency inside); candidate 7's verdict recorded either way; the layer is **spaces** (his model, ruled at the blessing — doc header carries the sketch) | — | Builder · opus-high | **LANDED** 2026-08-31 — the arrangement layer is one recursive structure over the register (`glass/spaces.ts`, pure and shared; `glass/arrangement.ts`, the parse boundary and the one write; the City moved to `glass/city.client.ts`): reorder, relabel, recolor and nest by pointer drag, persisted at `desk/city-arrangement.json` and read per poll, with the unfiled tail keeping every building the census knows. **Candidate 7 reproduced** on the live register — 24 buildings, 5 labels, **7 sections drawn** — and fixed by restoring B9's group-first invariant *through the same renderer his arrangement uses*. Item 0 (B26 F6, the scrim) landed first and `probes/drawer-click.probe.ts` stands over it. glass 621 → 657; findings F1–F11 in [B24](plans/b24-arrangement.md) |
