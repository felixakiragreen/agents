...
|---|---|---|---|---|
| C11 | [The turn cursor + the real seam](plans/c11-cursor-real-seam.md) — C7 F3's ruled fix (the transcript fallback answers the fired turn, cursor recorded at spawn) + the `{real: …}` subject arm `spawn.ts` reserves | C7 | Builder · opus-high | **LANDED** 2026-08-30 — all 5 bars evidenced in [C11](plans/c11-cursor-real-seam.md). **The F3 red was seen red first** (test committed as `cfb406a` before any fix; the parent engine read the resume's door death as `no report` — turn 0's verdict — and the fixed engine reads `dead`). The cursor is **recorded at spawn**, rides `ignited`/`resumed`, and re-derivation reads only past it. **Validated on real bytes, zero spend:** C4's two surviving multi-turn transcripts are now fixtures — `q2-a` (4 turns) and the **q5b summon round trip** (headless · a hand turn typed in a pane · headless), where turn arithmetic and the recorded cursor demonstrably disagree and the recorded one is right. The real seam parses `{real: {}}` strict, spawns `~/.local/bin/claude` with `cleanEnv` alone, and **nothing in the tree fires it**. Invariant 8 tightened to re-derive the torn-stream fallback past the cursor, seen to fail on a hand-planted stale cursor. Engine + barrage **92 tests green, both type gates exit 0**, `bun barrage/run.ts --runs 1000 --crashes 50` **exit 0 — 1000/1000 · 50/50 · 9/9, wall 147.9 s**. **Zero real `claude` invocations — budget 0 held.** **F1 C4's `captures/` hold streams, not transcripts** — the real transcripts live only in the account dirs and nothing in the repo pins them · **F2 the transcript reads a tool that merely failed as a denial**, measured on real bytes — it will ask Felix for a permission nobody refused, at C8 · **F3 a real arm-B user row carries array content, so `turns` undercounts — and the cursor does not care** (kill criterion answered NO) · **F4 the barrage does not reliably reproduce F3's defect** (400 cuts green with the fix sabotaged) — the guard is the two deterministic tests · F5 the classifier tax, one data point on the standing root entry, no new filing |
| C8 | [Real-session physics](plans/c8-real-session-physics.md) — layer 1: the same engine on real `claude -p`, small topologies ×3 accounts; campaign bar 4 + bar 5's cost inputs | C11 | Digger · opus-high | OPEN — laid 2026-08-30; **ignitable now** (C11 LANDED, its five gates pasted; verified at the batch-6 tender's review). **Budget ≤200 subject turns and ≤$15** (ignitions + resumes both count, C6 F8; either ceiling is a ⬡-fork, D21). Reads C11 F1 (the transcript corpus is in the account dirs, not `captures/`) and F2 (a failed tool re-derives as a denial) before it measures anything |
