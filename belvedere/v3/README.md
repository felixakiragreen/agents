...
| C8 | [Real-session physics](plans/c8-real-session-physics.md) — layer 1: the same engine on real `claude -p`, small topologies ×3 accounts; campaign bar 4 + bar 5's cost inputs | C11 | Digger · opus-high | IN FLIGHT 2026-08-30 — dispatched by the batch-6 tender on C11's verified gates; **budget ≤200 subject turns and ≤$15** (ignitions + resumes both count, C6 F8; either ceiling is a ⬡-fork, D21) |
