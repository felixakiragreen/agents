...
| C13 | [The report on disk](plans/c13-report-on-disk.md) — C8 F3's ruled fix: the completion rule reads the `StructuredOutput` pair as a closed turn, the fallback gains the report and **lands from disk**; the fake made faithful; C11's cursor signal restored | C8 | Builder · opus-high | OPEN — laid 2026-08-30; budget 0 real turns; precedes C9 — G4 does not convene on a falsified capability row |
...
C12's archive). Serial, no flow, no bulletin. Tender: Felix's word names it —
the batch-6 tender (the review session) stands ready to dispatch, or Felix
ignites by hand; kickoff in [C13](plans/c13-report-on-disk.md). C13 precedes
