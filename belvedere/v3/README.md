...
|---|---|---|---|---|
| C11 | [The turn cursor + the real seam](plans/c11-cursor-real-seam.md) — C7 F3's ruled fix (the transcript fallback answers the fired turn, cursor recorded at spawn) + the `{real: …}` subject arm `spawn.ts` reserves | C7 | Builder · opus-high | IN FLIGHT 2026-08-30 — dispatched by the batch-6 tender; budget 0 real turns |
| C8 | [Real-session physics](plans/c8-real-session-physics.md) — layer 1: the same engine on real `claude -p`, small topologies ×3 accounts; campaign bar 4 + bar 5's cost inputs | C11 | Digger · opus-high | OPEN — laid 2026-08-30; **budget ≤200 subject turns and ≤$15** (ignitions + resumes both count, C6 F8; either ceiling is a ⬡-fork, D21) |
