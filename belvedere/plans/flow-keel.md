# The flow keel — programmatic dispatch

**Status:** keel cut 2026-08-27 (Felix + Architect, the flow planning sitting) ·
**Chapter:** post-v0 — build rows cut at the v0 close sitting (Felix's word this
date, D11) · The parked **continuous-flow horizon** lands here; Felix's canon-inbox
...
Guild already writes every file it needs. Judgment lives IN sessions (Architect,
Builder, Digger); nothing between sessions requires a model. Mechanize the between,
keep the mantles, and Felix's attention narrows to what is genuinely his: gates,
escalations, taste. **Goal, his words: maximize time between Felix-gates.**

## 2. The audit — what a Dispatcher does, tested for judgment

1. Compose kickoffs → **mechanical** (fenced verbatim in orders + rider; the rail
   already resolves them).
2. Fire on dependency-LANDED → **mechanical** (board parse per D63 + census).
...
A **flow** is a declared DAG of steps. A step is a row id carrying: kickoff
(resolved from the order's fence), account, tier (from Staffing), venue (master |
worktree + branch), dependencies, permission mode (§5), and gate markers — a
Felix-card, or an **Architect sitting, itself a fireable step**, so review rides
the string. Storage format rides canon row 17 (the storage experiment, D7's
AI-native mandate) — this is a schema question, not markdown-prose parsing round 2.

## 4. The contract — D11

**The DAG is drawn, not listed (Felix, this sitting).** The flow page renders the
graph itself: nodes = steps (encapsulation-first labels, mantle colours, status
rings — the City View's vocabulary), edges = dependencies, parallel lanes side by
side, gates inline as Felix-cards, and the live census lights each node as it
runs — plan view and progress view are the same drawing. The same renderer can
later draw a board's Depends-on graph on building pages (parked, post-v0).

One click arms the flow; **the review of the rendered DAG is the authorization.**
The engine fires only declared steps through the existing hands (no new write
class — the fence stands), pauses at Felix-cards, on any ambiguity (**D10 binds
the engine wholesale**), and on HALT (its first consumer). Armed flows are
immutable: the Architect amends the flow file and unfired amended steps re-arm
with one click — which dissolves D57's relay dance. The arm view shows the bill:
...
3. **Concurrency lanes:** the flow file carries what batch notes carry — serial
   ...
   today).
4. **Liveness:** census `Stop` + `kill -0` (P1's two-sensor law, proven); step
   timeouts — everything has a limit.

## 6. The row-17 evidence pile

Five field asks now tell one story — doctrine's structure lives in prose and the
glass keeps needing it as data: `kind` (move/wave/fork), the fork's
`recommendation`, a row's `branch`, the baton's `holder`/precondition, a landing's
`holds`, and `encapsulation` (the 1–6-word canonical name, Felix's concept
2026-08-27). The storage experiment reads this keel as its case file; until it
rules, every one is derived render-side or paused on ambiguity, never guessed.

## 7. The channel

The Dispatcher mantle's fate is the Grand Architect's — the queued canon-inbox
entry plus this keel are the evidence, and v0 usage is the measure. Belvedere
builds the engine as glass features inside the existing fence; canon rules the
grammar fields and the mantle.

## 8. The Steward

The same engine, unattended — night-shift flows with HALT and the census as its
senses. Nothing new to design; the flow chapter IS the Steward's skeleton with
Felix awake. Unparked by his word only (D5 stands).

## Non-goals

An LLM between sessions · auto-arbitrage of accounts (explicit choice, usage
visible) · editing truth (forever-class, README §2) · replacing mantled judgment.
