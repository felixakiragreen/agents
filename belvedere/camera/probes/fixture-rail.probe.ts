...
export default async function (p: Probe): Promise<void> {
	...
	const wired = await p.count('article[data-kind="baton"][data-holder="session"] button[data-fire]');
	const his = await p.count('article[data-holder="felix"] button[data-fire]');
	...
	// his may reach `/hands/fire`, and the count is the assertion — a card cannot half-carry one.
	if (his !== 0) throw new Error(`${his} fire button(s) on Felix's cards — D10 says ambiguity never arms`);
	...
	console.log(`⬡ cards    ${gates} gates + ${hisBatons} baton · ${his} fire buttons between them`);
