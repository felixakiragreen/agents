...
**The deferred list carries its day, and nobody chooses it** (§4, the pilot survived at stigmergon G23). An entry's day is blamed off its own line — git's day, derived, because *a date he must choose per item is a tax he will not pay* (his ruling on the MEL ask, 2026-09-08) — and the building's **horizon** is read from its master doc's agreements (the paragraph naming *the deferral horizon*; thirty days when it names none, and the number is read as the record spells it, *thirty days* beside `30`). The board's count line says how many stand past it. Beside them the alarm: `board.deferred-drop` **warns** when the shelf shrank between the board doc's last two commits and the ledger of that span names neither `promoted` nor `deleted` — the loss class that swept ten live entries out of stigmergon's board at a bulk edit with no ledger word (G23-F5, restored from git). It lives in `lint` and not in `parse()`, because the parse is text in, values out and must never pay for a subprocess.
...
## The charge doc's header, and what the board owes it (050)

§5's header run is typed — `**Status:** … · **Depends on:** … · **Staffing:** …` — and with it the four slots the field asked for between 2026-09-09 and 2026-09-15 (032's stranded scope; stigmergon's canon asks at 086's lay):

| Slot | Reads |
|---|---|
| `**Parallel-safe with:** ‹charges›` | the ids, for the lay that put the charge in a parallel batch |
| `**Branch:** ‹name› from ‹base›` | the worktree and the branch it is cut from — a worktree on the wrong base is a false assumption (§10); a trailing parenthetical is an annotation for eyes, as §4's staffing rider is |
| `**Batch:** ‹shape› · ceiling ‹n› · gauge ‹text› · account ‹name›` | §4's batch note on a gate doc. **The members are not a slot** — they are the gate's Depends-on, by the edge test: a batch's membership and its dependency graph are one fact, and writing it twice is how the two drift apart |
| `tender: the dispatch` | §10's tender line, its own field. The engine's literal ignites; anything else names a hand and draws read-only, so two tenders never own one sum (stigmergon 086-F1 vi) |

A slot ends where the next `**Label:**` begins, so the skeleton's one-line run and a batch slot on its own line read the same. **The tender is read from the Mission and nowhere else** — the record writes it inside a code span, so the span is not masked, and a charge doc's Findings quote the form while ruling on it (stigmergon 086's own table). The form gates on **live**, as the kickoff arm does: §5's Branch slot was written 2026-09-08 and the record before it named the branch alone, so a spent doc's header parses and is never linted.

**The board's own typing follows it.** A Status annotation carries §4's typed remainders: `LANDED ‹date› — holds: ‹list›`, each hold an `E‹n›` or a `⬡ ‹text›`, the list running to the annotation's next em-dash and separated by `·` alone (a hold's text is free prose full of commas — belvedere's B22 lists five paid candidates inside one ⬡-hold). Escalations are ids, born `E‹n› — ‹what›` and dead `E‹n› ruled ‹date›`, merged across the building's boards; the deaths are read first, because one row writes both.

**`readiness` is §4's ignition rule, derived** — never kept, like the interest. A charge waits for each dependency to be LANDED; a **review gate waits for LANDED or KILLED** (*a documented kill is a result the gate reads* — stigmergon 086, 2026-09-09); a dependency carrying an **unresolved hold** pauses its dependants whatever its state, and a hold whose escalation the building has ruled is cleared. A `⬡-gate` dependency and a crossing stay in `waitingOn` by construction: the batch pauses at one, and the other names a door onto a board this parser does not read.
...
**`⬢‹n›` is the magnitude (D90) — his yes with its size**, and the floor at **1** is the whole of the reading: at 1 and above he looked, so the mark blesses; below it the thing proceeds and the review is owed, so the mark lands on the statement beside the words (`⬢0.1` IS the go-mark's rung). A **negative is a no** — it authorizes nothing and owes nothing, and reading it as credit would put a refusal on the statement as a debt he must pay. The magnitude is typed on the decision (`magnitude`) and `boot` prints the register's sized entries under its queue. The date is optional here and that is not D82's laxity: the standard dates the token only where the yes came later than the entry, and the entry carries its own day. **On the register the mark is read from the ATTRIBUTION and never from the prose** — D90 is the entry that defines the mark and writes a bare one into its own body, which is D82's *a definition is not a debt* at a distance the code mask cannot reach.
...
| a building's own graveyard | `WORDS.md` (DOCTRINE §3) — a word THIS building buried, re-minted on its own surfaces | `vocab.local-dead-word` (**warn**) |

A **warning** is reported and never enforced — §7's own word — and never moves the exit code.

**The building's register joins the arm (050).** `WORDS.md` is read in the standard's form: its bullets are the building's live words and its `dead · successor · when` table joins §9's for that building alone. A warning, not a failure, and the severity IS the difference: the standard's graveyard is the city's law, a building's is its own ruling, re-rulable at any sitting (D89). Three structural drops, each the arm's own law rather than a whitelist — **a word the register mints LIVE outranks its own row** (STANDARD §7's local kinds: stigmergon keeps `fold` for the reduce and buried it only as a UI verb, and `a run`, `a desk` and `the composer` are the same story), **a row qualifying its dead word in a parenthetical is killing a SENSE** and a sense cannot be patterned (*the paint (as a place)* — the live sense is the same letters, in *the file or the wire, never the paint*), and **a code-ticked cell is an identifier** (`WALL_MS`), whose rename is the compiler's alarm. The register itself is no law surface: it must name the dead to bury them, exactly as `canon/` must. Measured at stigmergon on the landing: **8 hits** — *the room* ×2, *the rail* ×2, *the wall*, *the history fold*, *encapsulation-first* — against 24 before the sense-kill drop. Birthplace: *rail*, killed at stigmergon D23 and re-minted twice with his blessing, with nothing in the city able to catch it.

**`docs/` is a law surface** and the arm walks it at any depth (050): DOCTRINE §3 makes it the home of durable distillations, which is what a law surface is — the master doc's overflow, read as law and respelled as law.

**The fence is structural, not a list of exceptions.** History and voice are masked out of the text before a pattern runs, so a dead word inside them is not *allowed*, it is not there: fenced code and summonses · blockquotes · inline code · `~~struck~~` text · link targets and one-word link texts · double-quoted spans · `Findings` / `Ledger` / `Decisions` sections · a board's Depends-on, Staffing and Status columns · **a LANDED or KILLED row whole** (a finished charge's title is the address its ledger cites) · closed charge docs · `LOG.md`, `SAPHO.md`, `dream.md` · `canon/` itself, which must name the dead to bury them · `WORDS.md`, which must do the same locally.
...
**The formula respell (050) — the currency law at the wording.** A pinned formula is one exact string (STANDARD §8), so its respell is a total substitution and nothing about it is a judgment: `formula.respell` rewrites formulas 8 and 26 as D89 respelled them — *never* → *not*, because the absolutes are reserved for what holds in every possible reading. It runs over every document the building keeps, **history included** (D81), and it declares no `changes`: like the id respell it earns the stronger law instead, every parsed field invariant under the table. Two refusals, both the converter's existing ones: a **ticked span is a form being NAMED** (047-F3) — the charge doc that commissioned the rule writes both spellings on one line with a `→` between them, and a rule that consumed its own commission would erase the record of what it changed — and **code is not a document**, because a string literal is the compiler's and `lexicon.ts`'s mirror has a louder alarm in `test/lexicon.test.ts`. This building ran it at 050 (8 edits across 6 files, a fixed point on the second pass); every other building at its next Architect session.
